...
/* 
* curPos is the index into buffer.
* startPos is the position in the file of the buffer
*/
static struct 
{
...
/*
* These need to be constructed once and then destroyed once.
...
Symbol lexerNextTok( void )
{
	...
	else
	{
		...
		/*
		* Check to see if symbol is already in our symbol table.
		* If not, then put it in.
		* All of our language keywords (if/while/do/etc) will all be in the
		* symbol table from the beginning.
		*/
...
int nextchar( void )
{
	...
	{
		/* 
		* We still want to increment the pointer so that 
...
Bool movepointer( int dx )
{
	...
	/* Check to see if we need to move the buffer: */
	if( buf.curPos + dx < 0 || buf.curPos + dx >= BUF_SIZE )
	{
		/*
		* Our new startPos will leave the pointer in the middle of the buffer
		* pointing at the same place in the file as before:
		*/
		startPos = buf.starPos + buf.curPos + dx - BUF_SIZE / 2;
		...
		/*
		* The file pointer is actually at the end of our buffer,
		* so we need to subtract BUF_SIZE:
		*/
		if( fseek( fData.inFile, starPos - ( buf.startPos + BUF_SIZE ), SEEK_CUR ) == 0 )
		{
